...
class SC_Event_List {
	...
	private function __construct() {
		...
		$this->atts = array(
			...
			'location_link'   => array( 'val'     => 'false<br />true<br />event_list_only<br />single_event_only',
			                            'std_val' => 'true',
			                            'visible' => false,
			                            'desc'    => 'This attribute specifies if the location should be made a google maps link'),
	...
	private function html_event( &$event, &$a, $single_day_only=false ) {
		...
		if( $this->is_visible( $a['show_location'] ) ) {
			if ($this->is_visible( $a['location_link'] )) {
				$out .= '<span class="event-location"><a target="_blcnk" href="https://maps.google.com/maps?ie=UTF8&gl=us&daddr='.esc_attr($event->location).'">'.esc_attr($this->db->truncate(min($max_length, $a['location_length']), $event->location)).'</a></span>';
			} else {
		...
		if( $this->is_visible( $a['show_cat'] ) ) {
			$out .= '<div class="event-cat">'.$this->html_category($event->categories, $a).'</div>';
		}
	...
	private function html_category($slug_text, $a)
	{
		$html = '';
		...
		sort($slug_array, SORT_STRING);
		foreach ($slug_array as $slug) {
			$html = '<a class="'.$slug.'" href="/'.$slug.'">'.esc_attr($this->categories->cat_array[$slug]['name']).'</a>';
		}
		return $html;
	}
