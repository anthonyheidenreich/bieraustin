...
function alkane_post_author() {
	...
	$output = sprintf( '<span class="entry-meta-sep"> &sdot; </span><span class="entry-author author vcard"><a href="%1$s" title="'. __( 'by %2$s', 'alkane' ) .'" rel="author">%2$s</a></span>', esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ), esc_html( get_the_author() ) );
	return $output;
...
function alkane_post_edit_link() {
	...
	edit_post_link( __( 'Edit', 'alkane' ), '<span class="entry-meta-sep"> &sdot; </span><span class="edit-link">', '</span>' );
	else:
	edit_post_link( __( 'Edit', 'alkane' ), '<span class="edit-link">', '</span>' );
	endif;
	$output = ob_get_clean();
...
function alkane_post_category() {
	...
	$output = sprintf( '<span class="cat-links"><span class="%1$s">'. __( 'Posted in:', 'alkane' ) .'</span> %2$s</span>', 'entry-utility-prep entry-utility-prep-cat-links', $categories_list );
	return $output;
}
...
function alkane_post_tags() {
	...
	$output = sprintf( '<span class="entry-meta-sep"> &sdot; </span><span class="tag-links"><span class="%1$s">'. __( 'Tagged:', 'alkane' ) .'</span> %2$s</span>', 'entry-utility-prep entry-utility-prep-tag-links', $tags_list );
	return $output;
}
...
function alkane_link_pages() {
	return wp_link_pages( array( 
		...
		'before' => '<div class="page-link"><span class="assistive-text">'. __( 'Pages:', 'alkane' ) .'</span>',
		'after' => '</div>',
		'link_before' => '<span>',
		'link_after' => '</span>',
		'echo' => 0
...
function alkane_post_style() {
	...
	if( $alkane_options['alkane_post_style'] == 'excerpt' ):		
		...
		the_content( __( 'Read More', 'alkane' ) . ' <span class="meta-nav">&rarr;</span>' );	
	endif;
...
<div id="loop-nav-numeric" class="nav-numeric">
  <h3 class="assistive-text"><?php _e( 'Post Navigation', 'alkane' ); ?></h3>
...
/** Alkane Loop Navigation Next/Prev */
function alkane_loop_nav_next_prev() {
	
	ob_start();
	next_posts_link( '<span class="meta-nav">&larr;</span> '. __( 'Older Posts', 'alkane' ) );
	$next_posts_link = ob_get_clean();
	
	ob_start();
	previous_posts_link( __( 'Newer Posts', 'alkane' ) .' <span class="meta-nav">&rarr;</span>' );
...
/** Alkane Loop Navigation Singular Post */
function alkane_loop_nav_singular_post() {
	
	ob_start();
	previous_post_link( '%link', '<span class="meta-nav">&larr;</span> '. __( 'Previous Post', 'alkane' ) );
	$previous_post_link = ob_get_clean();
	  
	ob_start();
	next_post_link( '%link', __( 'Next Post', 'alkane' ) . ' <span class="meta-nav">&rarr;</span>' );
...
/** Alkane Loop Navigation Singular */
function alkane_loop_nav_singular() {
	global $post;
?>
<div id="loop-nav-singular">
  <h3 class="assistive-text"><?php _e( 'Post Navigation', 'alkane' ); ?></h3>
  <div class="loop-nav-standard"><a href="<?php echo get_permalink( $post->post_parent ); ?>" rel="gallery"> <?php _e( '&larr; Return to', 'alkane' ); ?> <?php echo get_the_title( $post->post_parent ); ?></a></div>
</div><!-- end #loop-nav-singular -->
<?php
}

/** Alkane Loop Navigation Singular Attachment */
function alkane_loop_nav_singular_attachment() {
...
<div id="loop-nav-singlular-attachment" class="clearfix">
  <h3 class="assistive-text"><?php _e( 'Attachment Navigation', 'alkane' ); ?></h3>
  <div class="loop-nav-previous grid_6 alpha">
...
<div id="author-info" class="clearfix">
  ...
  <div id="author-description" class="grid_8 omega">    
      <h3><?php printf( __( 'About %s', 'alkane' ), get_the_author() ); ?></h3>
      <p><?php the_author_meta( 'description' ); ?></p>
      <div id="author-link">
        <a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author"><?php printf( __( 'View all posts by %s', 'alkane' ) . ' <span class="meta-nav">&rarr;</span>', get_the_author() ); ?></a>
      </div> <!-- #author-link	-->  
...
/** Alkane Footer */
add_action( 'alkane_footer', 'alkane_footer_init' );
function alkane_footer_init() {
...
?>
<div class="grid_5">
  <?php echo htmlspecialchars_decode ( esc_html ( $alkane_options['alkane_copyright'] ) ); ?>
</div>
<div class="grid_11">
  <?php _e( 'Designed by', 'alkane' )?> <a href="<?php echo $alkane_theme_data['AuthorURI']; ?>" title="WPAlkane">WPAlkane</a> &sdot; <a href="http://wordpress.org/" title="WordPress">WordPress</a>
</div>
<?php	
}

/** Alkane Comment List */
function alkane_comment( $comment, $args, $depth ) {
...
?>
			<li class="post pingback">
				<p><?php _e( 'Pingback:', 'alkane' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'alkane' ), '<span class="edit-link">', '</span>' ); ?></p>
		<?php
		break;
		default:
		?>
				...
				<div id="comment-<?php comment_ID(); ?>" class="comment">
					...
					<div class="comment-meta">
						<div class="comment-author vcard">
							...
							<?php edit_comment_link( __( 'Edit', 'alkane' ), '<span class="edit-link">', '</span>' ); ?>
		  
						</div> <!-- end .comment-author .vcard -->

						<?php if ( $comment->comment_approved == '0' ) : ?>
						<em class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'alkane' ); ?></em><br />
						<?php endif; ?>
					...
					<div class="reply">
						<?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Reply', 'alkane' ) . '<span>&darr;</span>', 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
					</div><!-- .reply -->
...
function alkane_content_more_link( $more_link, $more_link_text ) {
	return str_replace( $more_link_text, '<span>'. __( 'Read More &rarr;', 'alkane' ) .'</span>', $more_link );
}

/** Returns a "Read more" link for excerpts */
function alkane_continue_reading_link() {
	return '<span class="more-link-wrap"><a href="'. esc_url( get_permalink() ) . '" class="more-link"><span>'. __( 'Read More &rarr;', 'alkane' ) .'</span></a></span>';
}
...
function alkane_auto_excerpt_more( $more ) {
	return ' <span class="ellipsis">&hellip;</span> ' . alkane_continue_reading_link();
}	
...
function alkane_custom_excerpt_more( $output ) {
	...
	if ( has_excerpt() && ! is_attachment() ) {
		$output .= ' <span class="ellipsis">&hellip;</span> ' . alkane_continue_reading_link();
	}
	return $output;
